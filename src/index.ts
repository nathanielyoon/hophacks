    ...
    .then((spots) => {
      fetch("/spots", { method: "POST", body: JSON.stringify(spots) })
        .then((response2) => response2.json()).then((states) => {
          ...
          add("th", thead).textContent = "?",
            add("th", thead).textContent = "#";
          const tbody = add("tbody", table);
          for (let z = 0; z < spots.length; ++z) {
            row(tbody, spots[z], states[z]);
          }
          ...
          const submit = add("input", new_form, {
            type: "submit",
            value: "Add",
          });
          new_form.addEventListener("submit", (event) => {
            ...
            locate(key, alt_field.valueAsNumber).then((state) => {
              const spot = {
                label,
                lat: state.lat,
                lon: state.lon,
              ...
              row(
                tbody,
                { label, lat: state.lat, lon: state.lon, alt: state.alt },
                1,
              );
              fetch("/state", {
                method: "PUT",
                body: key + JSON.stringify(spots),
            ...
            function (this) {
              ...
              if (editing = !editing) {
                ...
                fetch("/state", {
                  method: "PUT",
                  body: key +
                    JSON.stringify(spots.filter((spot) => spot.label)),
                });
          ...
          check_in.addEventListener("submit", (event) => {
            ...
            locate(key, alt_input.valueAsNumber).then((state) =>
              fetch("/state", {
                method: "POST",
          ...
          add("button", undefined, { textContent: "Check out :(" })
            .addEventListener(
              "click",
              () =>
                fetch("/state", {
